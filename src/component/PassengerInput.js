  ...
  const handleSubmit = (e) => {
    if (state.nama.trim() && state.umur && state.jenisKelamin) {
      ...
      if (umur >= 75 || umur <= 12) {
        ...
        props.insertPengunjung({variables: {NAMA: state.nama ,UMUR: state.umur ,JENIS_KELAMIN: state.jenisKelamin, ID: state.id}})
        setState({
          ...state,
          nama: "",
          umur: "",
  ...
  return (
    <div>
      <div onSubmit={handleSubmit} style={viewMode}>
        ...
        <input type="number" className="input-text" placeholder="ID anda ..." value={state.id} name="id" onChange={onChange} />
